...
function roomState(room, forPlayerId = null) {
  return {
    ...
    civilWord: room.phase === "end" && room.wordsRevealed ? room.civilWord : undefined,
    undercoverWord: room.phase === "end" && room.wordsRevealed ? room.undercoverWord : undefined,
...
function getRoleInfo(room, playerId) {
  ...
  // Roles are hidden — only send the word, not the role label
...
function buildWordQueue(room) {
  ...
  // take wordsPerGame unique pairs
  return shuffled.slice(0, Math.min(room.settings.wordsPerGame, shuffled.length));
}
...
function startWordRound(room) {
  // Reset all players to alive for new word
...
function nextWordOrEnd(room, wordWinner) {
  ...
  if (hasMore) {
    // Brief results phase then next word
    room.phase = "word_end";
...
function resolveVote(room) {
  ...
  if (player) {
    ...
    room.lastEliminated = {
      ...
      isUndercover: player.isUndercover, isMrWhite: player.isMrWhite,
    };
...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (raw) => {
    ...
    if (msg.type === "create_room") {
      ...
      rooms[code] = {
        code, hostId: playerId, players: [player],
        phase: "lobby", round: 0, descriptionRound: 1,
        currentTurn: null, civilWord: null, undercoverWord: null,
        votes: {}, descriptions: {}, lastEliminated: null, winner: null,
        wordsRevealed: false, mrWhiteId: null, wordQueue: [],
        currentWordIndex: 0, customWords: [],
        settings: {
          nbUndercover: 1, nbMrWhite: 0,
          wordsPerGame: 3, descriptionsPerTurn: 1,
        },
      };
      playerRoom = code;
    ...
    if (msg.type === "submit_description") {
      ...
      const player = room.players.find(p => p.id === playerId);
      const key = `${playerId}_${room.descriptionRound}_${msg.descIndex || 0}`;
      ...
      const descriptionsPerTurn = room.settings.descriptionsPerTurn || 1;
      const myEntries = room.descriptions[playerId].entries.length;

      const alivePlayers = room.players.filter(p => p.alive);

      if (myEntries >= descriptionsPerTurn) {
        // Done with my turn — next player
        const allDone = alivePlayers.every(p => room.descriptions[p.id] && room.descriptions[p.id].entries.length >= descriptionsPerTurn);
        if (allDone) {
          room.phase = "vote";
          room.players.forEach(p => p.voted = false);
          room.votes = {};
